...
function uiRenderMixin(kbnServer, server, config) {
  ...
  // expose built css
  server.exposeStaticDir('/built_assets/css/{path*}', (0, _utils.fromRoot)('built_assets/css'));
  ...
  server.route({
    ...
    async handler(request, h) {
      ...
      const styleSheetPaths = [`${dllBundlePath}/vendors.style.dll.css`, `${regularBundlePath}/commons.style.css`, `${regularBundlePath}/${app.getId()}.style.css`, ...kbnServer.uiExports.styleSheetPaths.map(path => path.localPath.endsWith('.scss') ? `${basePath}/built_assets/css/${path.publicPath}` : `${basePath}/${path.publicPath}`).reverse()];
