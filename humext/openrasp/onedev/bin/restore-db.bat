...
:callcommand
if [%_COMMAND%]==[console] (
    %_WRAPPER_EXE% -c "%_WRAPPER_CONF%" wrapper.logfile.loglevel=NONE wrapper.console.title="OneDev Restore Database" wrapper.name=onedev_restore-db wrapper.displayname="OneDev Restore Database" wrapper.description="OneDev Restore Database" -- restore-db %_PARAMETERS%
