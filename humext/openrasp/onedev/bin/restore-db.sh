...
# Default-Stop: 0 1 6
# Short-Description: OneDev Restore Database
# Description: OneDev Restore Database
### END INIT INFO
...
APP_NAME="onedev_restore-db"
APP_LONG_NAME="OneDev Restore Database"
...
prepAdditionalParams() {
    ...
    if [ "X${PASS_THROUGH}" = "Xapp_args" -o "X${PASS_THROUGH}" = "Xboth" ] ; then
        if [ "X${PASS_THROUGH}" = "Xapp_args" ] ; then
            ADDITIONAL_PARA="wrapper.logfile.loglevel=NONE wrapper.console.title='OneDev Restore Database' wrapper.description='OneDev restore-db' -- restore-db"
