...
# Default-Stop: 0 1 6
# Short-Description: OneDev Backup Database
# Description: OneDev Backup Database
### END INIT INFO
...
APP_NAME="onedev_backup-db"
APP_LONG_NAME="OneDev Backup Database"
...
prepAdditionalParams() {
    ...
    if [ "X${PASS_THROUGH}" = "Xapp_args" -o "X${PASS_THROUGH}" = "Xboth" ] ; then
        if [ "X${PASS_THROUGH}" = "Xapp_args" ] ; then
            ADDITIONAL_PARA="wrapper.logfile.loglevel=NONE wrapper.console.title='OneDev Backup Database' wrapper.description='OneDev backup-db' -- backup-db"
