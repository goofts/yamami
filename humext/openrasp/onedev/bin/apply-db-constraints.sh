...
#       Configuration GUI.
### BEGIN INIT INFO
# Provides: onedev_apply-db-constraints
...
# Default-Stop: 0 1 6
# Short-Description: OneDev Apply DB Constraints
# Description: OneDev Apply DB Constraints
### END INIT INFO
...
APP_NAME="onedev_apply-db-constraints"
APP_LONG_NAME="OneDev Apply DB Constraints"
...
prepAdditionalParams() {
    ...
    if [ "X${PASS_THROUGH}" = "Xapp_args" -o "X${PASS_THROUGH}" = "Xboth" ] ; then
        if [ "X${PASS_THROUGH}" = "Xapp_args" ] ; then
            ADDITIONAL_PARA="wrapper.logfile.loglevel=NONE wrapper.console.title='OneDev Apply DB Constraints' wrapper.description='OneDev apply-db-constraints' -- apply-db-constraints"
